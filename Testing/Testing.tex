...
\textbf{2.) Search Employee} \\
Pre conditions: Valid employee number provided \\
Post condition: Employee details returned \\
Test Case 1: \$scope.searchEmployee() should send an error message if the employee does not exist in the database: Pass \\
Test Case 2: \$scope.searchEmployee() should find the employee if an existing employeeID was typed in: Pass \\
\textbf{3.) User History} \\ 
Test Case 1: \$scope.getUserHistory() should get the history of the employee that's signed in: Pass \\
Test Case 2: \$scope.getUserHistory() should default to this month if no date provided and get history: Pass
...
\textbf{5.) Create a menu category} \\ 
...
\textbf{9.) Add to plate}\\
Pre conditions: Item on menu and in stock \\
Post condition: Item added to plate (saved in cookie)\\
Test Case 1: \$scope.addToPlate() should add menu item to plate \\
...
Pre Conditions: User has been found, user ID exits in the database if generating report for user\\
Post Conditions: Report has been downloaded in pdf format for either specific user or all users\\
Test Case 1: \$scope.generateReport() should not generate report if missing fields: Pass\\
Test Case 2: \$scope.generateReportAll() should not generate report if missing fields: Pass \\
...
Test Case 1: \$scope.placeOrder() should place order: Pass\\
Test Case 2: \$scope.placeOrder() should NOT place order if user is not signed in: Pass\\

\textbf{2.) Remove from plate} \\
Pre Conditions: Item is on Plate \\
Post Conditions: Item is removed from plate \\
Test Case 1: \$scope.removeFromPlate should remove item from plate: Pass \\
