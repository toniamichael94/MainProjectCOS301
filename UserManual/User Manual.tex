...
\end{figure}
 ...
 \textbf{Version} & 0.0.7 \\
 \hline
 \textbf{Document Status} & Seventh Version - edited according to feedback, edited cashier page/ images, added reporting and dynamic categories and updated older images\\
 \hline
 \end{tabu}
...
\begin{figure}[H]
  \centering
    ...
    \caption{The admin page - superuser can view the audit trail for all actions performed in the system for a specified period. } 
\end{figure}

\begin{figure}[H]
  \centering
    \includegraphics[width=1.0\textwidth]{screenshots/Menu_001.png}
    \caption{The admin page - the dropdown menu for the field to select audit details contains the following actions, more specifically all the main actions a user can perform in the system} 
\end{figure}

\begin{figure}[H]
  \centering
    \includegraphics[width=1.0\textwidth]{screenshots/auditRole.png}
    \caption{The admin page - if the superuser selects one of the options, the audit trail is limited to that option} 
\end{figure}

\subsection{Superuser: The "Branding Settings" Page} 
There are two sections on this page. One where the user can change the canteen name, by merely typing in a new name over the old name in the allocated textbox, and another where the superuser can upload a new cover photo for the system. The superuser can also configure the colour scheme for the system.
