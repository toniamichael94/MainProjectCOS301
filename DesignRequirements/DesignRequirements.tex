...
\title{ \rule{\textwidth}{1pt}  \\ \Huge Design Requirements \\ 
...
\textbf{Version} & 0.0.4 \\
\hline
\textbf{Document Status} & Fourth Version - added all the functions for phase 5 sprint and edited the document \\
\hline
\end{tabu}
...
\centering
 ...
 \hline\hline
 0.0.1 & 29 May 2015 &  First draft contains & Rendani Dau, \\ & &  first two use cases & Elana Kuun, \\ & &  template methods and  & Semaka Malapane, \\ & & declarations & Antonia Michael \\ & & & Isabel Nel \\ & & & \\
 \hline 
 & & & \\
 0.0.2 & 27 August 2015 &  Second draft adding & Rendani Dau, \\ & & all the client & Elana Kuun, \\ & & and controllers for & Semaka Malapane, \\ & & the updated system &  Antonia Michael \\ & & & Isabel Nel \\   [1ex] 
 \hline 
 & & & \\
 0.0.3 & 24 September 2015 &  Third draft adding & Rendani Dau, \\ & &  all the reporting & Elana Kuun, \\ & &  and dynamic pages  & Semaka Malapane, \\ & & functionality &  Antonia Michael \\ & & & Isabel Nel \\   [1ex] 
 \hline  
 & & & \\
 0.0.4 & 9 October 2015 &  Fourth draft adding & Rendani Dau, \\ & & the functions for & Elana Kuun, \\ & & the phase5 sprint  & Semaka Malapane, \\ & & functionality &  Antonia Michael \\ & & & Isabel Nel \\   [1ex] 
 \hline  
 \end{tabular}
\end{table}
...
\item Methods: 
	\begin{itemize}
		...
		\item Load employees \\ \$scope.loadEmployees = function() 
		\\ \\Usage: This function is used to dynamically populate the drop down menu for the change employee ID functionality
		\item Set theme \\ \$scope.setThemeName = function() 
		\\ \\ Usage: Function to get theme that user desires to send to the server side so that the CSS file can be replaced. 
	\end{itemize}
\end{enumerate}
%%superuser server controller---------------------------------------------
...
\item Methods:
	\begin{itemize}
		...
		\item Get audits \\ exports.getAudits = function(JsonObject req, JsonObject res) 
		\\ \\ Usage: Function to trace audit trail for a specified time period.
		\item Get audit types \\ exports.getAuditTypes = function(JsonObject req, JsonObject res) 
		\\ \\ Usage: Function to get specific types that the user wants to view the audit trail for.
		\item Get theme name \\ exports.setThemeName = function(JsonObject req,JsonObject res)
		\\ \\	Usage: Function to take in the theme specified by the user, and to copy the contents of the desired CSS file into the system's css file.
	\end{itemize}
\end{enumerate}
%manage cafeteria
...
\item Methods:
	\begin{itemize}
		...
		This function sets the help variable to true, to indicate that a user wants help alerts.
		 \item Search bar dynamic\\
		\$scope.searchBarDynamic = function(int row) \\
		-Usage \\ \\
		Function to search menu items from the dynamically generated category pages.
		\item Search bar \\
		\$scope.searchBar = function (int row)\\
		-Usage \\ \\
		This function searches through all the different menu categories to provide found menu items.
		\item Search menu category \\
		\$scope.searchMenuCategory = function(Boolean isValid) \\
		-Usage \\ \\
		This function is used to locate a menu item in order to either delete or update it.
		\item updateMenuCategory \\
		\$scope.updateMenuCategory = function(Boolean isValid)\\ 
		-Usage \\ \\ 
		This function is used to update the category name for all the menu items. It updates the name with the value entered in the corresponding textbox by the user.
		\item deleteMenuCategory \\
		\$scope.deleteMenuCategory = function() \\
		-Usage \\ \\
		This function is used to locate the category name entered by the user in the corresponding textbox so that it can be removed in the database on the server side.
		\item generateSoldReport\\
		\$scope.generateSoldReport = function() \\ 
		-Usage \\ \\
		Function to generate a bill of orders  and totals in a repor, for the specified time period.
		\item addSoldItem\\
		...
		\$scope.generateReport = function() \\
		-Usage \\ \\
		Function to generate a report of most popular categories for the specified time period.
		\item generatePopularReport\\
		\$scope.generatePopularReport = function() \\
		-Usage \\ \\
		Function to generate a report of most popular menu items for the specified time period.
...
\item Methods:
	...
	\item Search menu category  \\ exports.searchMenuCategory = function(JsonOBject req, JsonOBject res) \\ \\ 
Usage \\
This function is used to search for menu items on the sub menu pages i.e. the pages generated for each menu category.
	\item Update menu category \\ exports.updateMenuCategory = function(JsonOBject req, JsonOBject res)
\\ \\
Usage \\ 
This function is used to locate the old category name and update it to the new name, saving this new result in the database.
\item Update menu category items \\  exports.updateCategoryMenuItems = function (JsonOBject req, JsonOBject res)
\\ \\
Usage \\
This function is used to update the category name for all the menu items.
\item  Delete Menu Category \\ exports.deleteMenuCategory = function (JsonOBject req, JsonOBjectres)
\\ \\
Usage \\ 
This function is used to locate the category name entered and to remove it from the database.
\item Upload Image \\ exports.uploadImage = function(JsonOBject req, JsonOBject res)
\\ \\ 
Usage \\
This method is used to store the path of the uploaded image in the database.
\item Generate Sold Report \\ exports.generateSoldReport = function(JsonOBject req,JsonOBject res)
\\ \\
Usage \\
Function to generate a bill of orders  and totals in a repor, for the specified time period.
\item Generate Report \\ exports.generateReport = function(JsonOBject req,JsonOBject res)
\\ \\
Usage \\
Function to generate a report of most popular categories for the specified time period.
\item Generate Popular Report \\ exports.generatePopularReport = function(JsonOBject req,JsonOBject res)
\\ \\
Usage \\
Function to generate a report of most popular menu items for the specified time period.
	\end{itemize}
